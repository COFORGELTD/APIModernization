...
- Count of operations (INCLUDING imported WSDLs)
- Import/include references AND their targets
- SOAP binding details
- **CRITICAL**: Resolve and follow all WSDL imports/includes to find operations in dependent WSDLs
...
- **Imports/Includes Found**: [YES/NO - list each with file location and namespace]
  - Import: [Filename] (namespace: [ns], location: [path])
  - Include: [Filename] (schema location: [path])
  - [Follow and resolve EACH import/include to its actual file location]
...
### Summary
- **Total Operations** (including all imported WSDLs): [NUMBER] ← COUNT MUST INCLUDE IMPORTED OPERATIONS
- **Direct Operations** (from this WSDL only): [NUMBER]
- **Imported Operations** (from imported WSDLs): [NUMBER]
...
## Step 2a: Recursive WSDL Resolution (CRITICAL FOR COMPLETE DOCUMENTATION)

### Task
Resolve all WSDL import chains and dependencies. Many WSDLs reference other WSDLs via `<import>` or `<include>` elements. You MUST follow these chains to document ALL operations, even those in imported/included WSDLs.

### Processing Rules

1. **Find all import/include elements** in each WSDL:
   ```xml
   <import namespace="..." location="path/to/other.wsdl"/>
   <include schemaLocation="path/to/schema.xsd"/>
   ```

2. **Resolve relative paths** - If a WSDL imports `../shared/Common.wsdl`, locate it:
   - Check relative to current WSDL's directory
   - Check common shared folders (shared/, common/, lib/)
   - Check parent directory structure

3. **Document full dependency chain**:
   ```
   LoanService.wsdl
   ├── imports PaymentService.wsdl
   │   ├── includes Common.xsd
   │   └── imports CoreTypes.wsdl
   │       └── includes Enumerations.xsd
   └── includes LoanTypes.xsd
   ```

4. **Parse imported WSDLs fully** - Don't skip them! Extract:
   - All operations from imported services
   - All types from imported schemas
   - All faults defined in imported WSDLs
   - Namespace relationships

5. **Flatten the namespace hierarchy** - Create consolidated documentation showing:
   - Which operations come from which WSDL file
   - Which types come from which schema
   - All fully qualified type names
...
**Current WSDL**: [Filename]

**Direct Dependencies**:
- Imports:
  - [Other.wsdl] (namespace: ...)
  - [Shared.wsdl] (namespace: ...)
- Includes:
  - [Common.xsd] (for types)
  - [Enumerations.xsd] (for enums)

**Transitive Dependencies** (imported by imports):
- [PaymentService.wsdl] → [Common.xsd]
- [CoreTypes.wsdl] → [Enumerations.xsd]

**Total Operations Including Imports**: [NUMBER]
- From [CurrentWSDL.wsdl]: [N] operations
- From [ImportedWSDL1.wsdl]: [N] operations  
- From [ImportedWSDL2.wsdl]: [N] operations

**Total Types Including Imports**: [NUMBER]
- From [CurrentWSDL.wsdl]: [N] types
- From [ImportedSchema.xsd]: [N] types
```

### Critical Notes for Step 3 Forward

- When documenting operations in Step 3, **ALWAYS include the source WSDL file name** even if imported
- When documenting complex types in Step 4, **show the XSD/WSDL source file** where each type is defined
- For operations that use imported types, **document the full type path**: `urn:Namespace:TypeName (from ImportedSchema.xsd)`
- Track transitive dependencies - operations may use types from WSDLs not directly imported
